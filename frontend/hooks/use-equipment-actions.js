...
import { API_IMAGES, API_EQUIPMENTS, API_EQUIPMENT_DETAIL } from "@/constants/api";
import { authFetch } from "@/lib/auth-fetch";

function parseEquipmentError(data, fallback = "Gagal menyimpan equipment.") {
...
export function useEquipmentActions() {
  ...
  const updateEquipment = async (id, payload) => {
    ...
    try {
      ...
      const resp = await authFetch(API_EQUIPMENT_DETAIL(id), {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
      ...
      let message = "Gagal memperbarui equipment. Periksa data dan coba lagi.";
      try {
        const data = await resp.json();
        message = parseEquipmentError(data, message);
      } catch (_) {
