...
export function EquipmentForm({ equipmentId }) {
  ...
  useEffect(() => {
    ...
    const loadDetail = async () => {
      setErrorMessage("");
      try {
        const resp = await authFetch(API_EQUIPMENT_DETAIL(equipmentId));
        ...
        setFormData({
          ...
          roomId: data.room || "",
          isMoveable: String(Boolean(data.is_moveable)),
          imageFile: null,
    ...
    <section className="space-y-4">
      <div className="mx-auto text-center">
        <p className="text-sm text-muted-foreground">
          {equipmentId
            ? "Perbarui data equipment dan pilih ruangan penempatan."
