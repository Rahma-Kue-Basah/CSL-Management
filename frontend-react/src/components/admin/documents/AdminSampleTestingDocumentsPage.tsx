...
import { useAdminPengujianDocuments } from "@/hooks/admin/use-admin-pengujian-documents";
import { useHistoryRequesterOptions } from "@/hooks/history/use-history-requester-options";
import { type PengujianDocumentType } from "@/hooks/pengujians/use-pengujians";
...
export default function AdminSampleTestingDocumentsPage({
  ...
  const activeDocumentTypes =
    documentTypeFilter
      ? [documentTypeFilter as PengujianDocumentType]
  ...
  const groupedItems = useMemo(
    () =>
      groups.map((group) => ({
        groupKey: String(group.pengujianId),
        pengujianId: String(group.pengujianId),
        code: group.code,
  ...
  const allVisibleSelected =
    ...
    groupedItems.every((item) => selectedIds.includes(item.pengujianId));
  const someVisibleSelected =
    groupedItems.some((item) => selectedIds.includes(item.pengujianId)) &&
    !allVisibleSelected;
  ...
  useEffect(() => {
    setSelectedIds((prev) =>
      prev.filter((id) => groupedItems.some((item) => String(item.pengujianId) === String(id))),
    );
  }, [groupedItems]);
  ...
  const toggleSelectAllVisible = (checked: boolean) => {
    if (!checked) {
      setSelectedIds((prev) =>
        prev.filter((id) => !groupedItems.some((item) => item.pengujianId === id)),
      );
      return;
    }
    ...
    setSelectedIds((prev) => {
      ...
      groupedItems.forEach((item) => next.add(item.pengujianId));
                  ...
                  <tr className="border-b bg-slate-50/70">
                    <td className="px-3 py-2 text-center">
                      <input
                        ...
                        checked={selectedIds.includes(group.pengujianId)}
                        onChange={() => toggleItemSelection(group.pengujianId)}
                      />
                    </td>
