...
export function useDeleteAnnouncement() {
  ...
  const deleteAnnouncement = async (id: string | number) => {
    ...
    try {
      const result = await announcementsService.remove(id);
      ...
      const detail =
        typeof (payloadData as { detail?: string })?.detail === "string"
          ...
          : `Gagal menghapus pengumuman (${result.status})`;
      const message = detail || `Gagal menghapus pengumuman (${result.status})`;
  ...
  const bulkDeleteAnnouncements = async (ids: Array<string | number>) => {
    ...
    try {
      const result = await announcementsService.bulkRemove(ids);
      ...
      if (!result.ok) {
        const message =
          payload.detail || `Gagal menghapus pengumuman (${result.status})`;
        setErrorMessage(message);
        return { ok: false as const, message };
      }
